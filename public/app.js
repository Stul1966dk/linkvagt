...
function siteCell(site) {
  ...
  return `<div class="site-cell"><span class="favicon">${escapeHtml(site.name.slice(0, 1))}</span><span><strong>${escapeHtml(site.name)}</strong><small>${escapeHtml(host)}</small></span></div>`;
}
...
function renderMetrics() {
  ...
  const metrics = [
    ...
    ['Scanner', current ? `${current.links_checked || 0}/${current.pages_total || '?'}` : 'Klar', current ? 'En scanning er i gang' : 'Venter på næste opgave']
  ];
...
function renderOverviewSites() {
  ...
  $('#overview-sites').innerHTML = sites.length ? sites.map((site) => {
    const status = site.latest_scan_status === 'running' ? ['running', 'I gang'] : site.broken_count ? ['broken', 'Problemer'] : ['ok', site.latest_scan_id ? 'OK' : 'Ikke scannet'];
    return `<tr>${`<td>${siteCell(site)}</td>`}<td><span class="badge ${status[0]}">${status[1]}</span></td><td>${issueCounts(site)}</td><td>${relativeDate(site.last_scan_at)}</td><td><div class="row-actions"><button class="icon-button" data-action="scan" data-id="${site.id}" title="Start scanning" aria-label="Start scanning">↻</button><button class="icon-button" data-action="open-latest" data-id="${site.id}" title="Se seneste resultat" aria-label="Se seneste resultat">→</button></div></td></tr>`;
...
function renderActivity() {
  ...
  $('#recent-activity').innerHTML = scans.length ? scans.slice(0, 6).map((scan) => `<div class="activity"><span class="activity-dot ${scan.status}"></span><div><strong>${escapeHtml(scan.site_name)}</strong><small>Manuel scanning</small></div><span class="badge ${scan.status}">${statusName(scan.status)}</span><time>${relativeDate(scan.created_at)}</time></div>`).join('') : '<div class="empty"><strong>Ingen aktivitet</strong>Scanninger vil blive vist her.</div>';
}
...
function renderScans() {
  ...
  $('#scans-table').innerHTML = scans.length ? scans.map((scan) => {
    ...
    const scanType = { page: 'Enkeltside', link: 'Enkelt link', site: 'Hele websitet' }[scan.mode] || scan.mode;
...
function renderFindings() {
  ...
  const filtered = sortFindings(state.findings.filter((finding) => {
    ...
    const text = `${finding.destination_url} ${finding.final_url || ''} ${finding.sources.map((source) => source.source_url).join(' ')}`.toLowerCase();
  ...
  $('#findings-table').innerHTML = filtered.length ? filtered.map((finding) => {
    const sources = finding.sources.slice(0, 2).map((source) => `<a href="${escapeHtml(source.source_url)}" target="_blank" rel="noreferrer">${escapeHtml(source.source_url)}</a>`).join('');
    const more = finding.sources.length > 2 ? `<small>+ ${finding.sources.length - 2} kildesider</small>` : '';
    ...
    return `<tr class="${finding.ignored ? 'ignored-row' : ''}"><td>${categoryBadge(finding)}${finding.ignored ? '<small>Ignoreret</small>' : ''}</td><td><div class="destination"><strong><a href="${escapeHtml(finding.destination_url)}" target="_blank" rel="noreferrer">${escapeHtml(finding.destination_url)}</a></strong>${redirect}</div></td><td><div class="source-list">${sources}${more}</div></td><td>${fmtDate(finding.first_seen_at, false)}</td><td><div class="row-actions">${canRepair ? `<button class="button primary" data-action="repair" data-finding="${finding.id}">Ret link</button>` : ''}${finding.ignored ? `<button class="button secondary" data-action="unignore" data-rule="${finding.ignore_rule_id}">Aktivér</button>` : finding.category !== 'ok' ? `<button class="button secondary" data-action="ignore" data-finding="${finding.id}">Ignorer</button>` : ''}</div></td></tr>`;
...
async function openResults(scanId) {
  ...
  $('#csv-link').href = `/api/export.csv?scan_id=${scan.id}`;
...
function renderDiagnostics() {
  ...
  $('#diagnostic-list').innerHTML = items.length ? items.map((item) => {
    const type = item.event_type === 'scan_failed' ? 'Scanning fejlede'
      : item.event_type === 'page_fetch_failed' ? 'Kildeside kunne ikke hentes'
        : item.status_code ? `HTTP ${item.status_code}` : 'Netværksfejl';
    const attempts = item.attempt_count > 1
      ? `<span>${item.verification_status === 'confirmed' ? 'Bekræftet' : item.verification_status === 'recovered' ? 'Kom sig' : 'Ustabil'} · ${item.attempt_count} forsøg</span>`
      : '';
    return `<article class="diagnostic-item ${escapeHtml(item.severity)}">
      ...
      <div class="diagnostic-meta">${attempts}${item.scan_id ? `<span>Scanning #${item.scan_id}</span>` : ''}</div>
    </article>`;
...
document.addEventListener('click', async (event) => {
  ...
  if (nav) {
    ...
    if (nav.dataset.view === 'settings') {
      Promise.all([loadMailSettings(), loadExclusionSettings(), loadBackupStatus()]).catch((error) => toast(error.message, 'error'));
    }
  ...
  try {
    ...
    if (action.dataset.action === 'open-latest') {
      if (!site.latest_scan_id || site.latest_scan_status !== 'completed') toast('Hjemmesiden har endnu ikke et færdigt resultat');
      else await openResults(site.latest_scan_id);
    }
...
$('#site-form').addEventListener('submit', async (event) => {
  ...
  delete data.id;
  try {
    await api(id ? `/api/sites/${id}` : '/api/sites', { method: id ? 'PUT' : 'POST', body: JSON.stringify(data) });
    $('#site-dialog').close();
    await loadBase();
...
async function saveWordpressConnection() {
  ...
  delete data.site_id;
  await api(`/api/sites/${siteId}/wordpress`, { method: 'PUT', body: JSON.stringify(data) });
  return siteId;
}
...
async function saveMailSettings(showConfirmation = true) {
  ...
  await api('/api/settings/mail', { method: 'PUT', body: JSON.stringify(data) });
